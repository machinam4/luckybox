...
const { deposit } = require("./TransactionController");
...
const stkpush = async (amount, player) => {
  try {
...
const expressSTK = async (request, response) => {
  ...
  const playerResponse = request.body.Body.stkCallback;
  const transaction = await PaybillStk.findOne({
    CheckoutRequestID: playerResponse.CheckoutRequestID,
  });
  // console.log(transaction);
  if (playerResponse.ResultCode === 1032) {
    transaction.ResultCode = playerResponse.ResultCode;
    await transaction.save();
    // send sms transaction failed
  } else {
    const transactionItem = playerResponse.CallbackMetadata.Item;
    transaction.ResultCode = playerResponse.ResultCode;
    ...
    await deposit(payments);
    // console.log(account);

    await account.save();
    //console.log(account.balance);
    // emit player deposit seccefully
